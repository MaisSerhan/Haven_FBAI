...
def extract_audio_features(audio_path, n_mfcc=13):
    try:
        # Use soundfile for loading audio, which handles various formats
...
def process_audio_dataset(extract_path):
    ...
    # Define the audio extensions you want to include (case-insensitive)
    AUDIO_EXTENSIONS = ('.wav', '.mp3', '.ogg', '.flac', '.m4a') # Added .m4a
    ...
    for root, _, files in os.walk(extract_path):
        ...
        for file in files:
            ...
            # Check if the file ends with any of the specified audio extensions
    ...
    if len(data) == 0:
        send_training_update("No supported audio files found in the dataset. Please ensure your dataset contains files with extensions like .wav, .mp3, .ogg, .flac, or .m4a.")
        return None
    ...
    if len(all_feature_rows) == 0:
        send_training_update("No features could be extracted. Please check your dataset and ensure it contains valid audio files.")
        return None
