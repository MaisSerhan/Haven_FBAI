...
function checkModelStatus() {
    const checkModelUrl = `${window.location.origin}/check-model`;
    fetch(checkModelUrl)
...
function setupTrainingEventSource() {
    const eventSource = new EventSource(`${window.location.origin}/training-updates`);
...
function handleDatasetUpload(event) {
    ...
    fetch(`${window.location.origin}/train-model`, {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
function submitRecordedAudio(audioBlob) {
    ...
    fetch(`${window.location.origin}/analyze-live`, {
        method: 'POST',
        body: formData
    })
        .then(response => response.json())
